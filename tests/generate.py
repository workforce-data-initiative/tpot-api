...
def generate(n):
    ...
    while total < n:
        name = (fake.last_name() + " " +
                np.random.choice(["School", "Institute", "College",
                                  "University"]))
        provider_id = id
        id += 1
        ...
        for _ in range(int(random.expovariate(1/10)+1)):
            ...
            for year in np.unique(np.random.choice(range(2014, 2017), 3)):
                ...
                d['all_exited'].append(int(d['all_served'][-1] *
                                       (random.random()+1)/2))

                d['wioa_served'].append(int(d['all_served'][-1] *
                                        (random.random()+1)/2))
                d['wioa_exited'].append(int(min(d['wioa_served'][-1],
                                                d['all_exited'][-1]) *
                                            (random.random()+1)/2))
                d['wioa_ita_served'].append(int(d['wioa_served'][-1] *
                                            (random.random()+1)/2))
                d['wioa_ita_exited'].append(int(min(d['wioa_ita_served'][-1],
                                                    d['wioa_exited'][-1]) *
                                                (random.random()+1)/2))
                d['all_completed'].append(int(d['all_exited'][-1] *
                                              (random.random()+1)/2))
                d['wioa_completed'].append(int(d['wioa_exited'][-1] *
                                               (random.random()+1)/2))

                d['all_employed_2q'].append(int(d['all_served'][-1] *
                                            (random.random()+1)/2))
                d['all_employed_4q'].append(int(d['all_served'][-1] *
                                            (random.random()+1)/2))
                d['all_median_earnings'].append(random.randint(10000, 100000))

                d['all_obtained_credential'].append(int(
                    d['all_completed'][-1] * (random.random()+1)/2))
                d['wioa_obtained_credential'].append(int(
                    d['wioa_completed'][-1] * (random.random()+1)/2))
                d['nonwioa_employed_2q'].append(int(
                    (d['all_exited'][-1]-d['wioa_exited'][-1]) *
                    (random.random()+1)/2))
                d['nonwioa_employed_4q'].append(int(
                    (d['all_exited'][-1]-d['wioa_exited'][-1]) *
                    (random.random()+1)/2))
                d['nonwioa_median_earnings'].append(
                    random.randint(10000, 100000))
                d['nonwioa_credential_rate_den'].append(int(
                    (d['all_exited'][-1]-d['wioa_exited'][-1]) *
                    (random.random()+1)/2))
                d['nonwioa_credential_rate_num'].append(int(
                    d['nonwioa_credential_rate_den'][-1] *
                    (random.random()+1)/2))
                d['cost_per_wioa_participant'].append(
